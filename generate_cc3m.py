...
def main(args):
    ...
    all_dataset = CC3MFeatures(path='/weka/oe-training-default/georges/datasets/cc3m256_featuresv3/', mode='val')
    ...
    captions_sample_folder_dir = f"{args.sample_dir}/{folder_name}_captions"
    if accelerator.is_main_process:
        os.makedirs(sample_folder_dir, exist_ok=True)
        os.makedirs(real_sample_folder_dir, exist_ok=True)
        os.makedirs(captions_sample_folder_dir, exist_ok=True)
        print(f"Saving .png samples at {sample_folder_dir}")
        print(f"Saving captions at {captions_sample_folder_dir}")
    dist.barrier()
