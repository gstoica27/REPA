...
do
    ...
    SAVE_DIR="/weka/oe-training-default/georges/samples/stacked-cfg-search/fid_50k/"$3"_steps/with_cfg/with_cfg_"$1"_"$2"/${exp_name}"
    for fname in $CHECKPOINT_ITERS
        do
        if [ ! -d "${SAVE_DIR}/${exp_name}" ]; then 
            torchrun \
