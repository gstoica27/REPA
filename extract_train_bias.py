...
def main(args,):    
    ...
    if accelerator.is_main_process:
        os.makedirs(args.save_dir, exist_ok=True)  # Make results folder (holds all experiment subfolders)
    ...
    if accelerator.is_main_process:
        print(f"Dataset contains {len(train_dataset):,} images ({args.data_dir})")
    ...
    if accelerator.is_main_process:
        # logger.info(f"Dataloader has {len(train_dataloader.dataset)} images.")
    ...
    estimated_bias = torch.zeros((4, 32, 32), dtype=torch.float64).to(device)
    for epoch in range(args.epochs):
        ...
        for raw_image, x, y in train_dataloader:
            ...
            with torch.no_grad():
                ...
                #         zs.append(z)
            gathered_bias =  accelerator.gather(
    ...
    if accelerator.is_main_process:
        logger.info("Done!")
        save_path = os.path.join(args.save_dir, "sampled_latent_bias.pt")
        logger.info("Saving bias to: ", save_path)
        torch.save(
            estimated_bias.to(torch.float32),
            save_path
        )
